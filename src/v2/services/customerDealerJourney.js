...
const cleanId=v=>String(v||'').trim();const productIdOf=product=>cleanId(product?.id)||null;const dealerIdOf=dealer=>cleanId(dealer?.dealer_id)||null;const EVENT_TYPES=new Set(['PROFILE_VIEW','DEALER_SELECTED','CALL_CLICK','WHATSAPP_CLICK','DIRECTIONS_CLICK']);
...
export async function openCustomerDealerProfile(dealer,product=null,enquiryId=null){const dealerId=dealerIdOf(dealer);if(!dealerId)throw new Error('DEALER REQUIRED');const profile=await loadDealerProfile(dealerId);if(!profile)throw new Error('VERIFIED DEALER PROFILE NOT AVAILABLE');await safeTrack({dealer:profile,eventType:'PROFILE_VIEW',product,enquiryId});return profile}
export async function selectCustomerDealer(dealer,product=null,enquiryId=null){if(!dealerIdOf(dealer))throw new Error('DEALER REQUIRED');await safeTrack({dealer,eventType:'DEALER_SELECTED',product,enquiryId});return dealer}
