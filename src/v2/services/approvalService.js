...
export const approvalService={
 ...
 async decide(id,decision,note='',module=''){const name=module==='payment'?'decide_payment_approval':'decide_approval_request',args=module==='payment'?{p_approval:id,p_decision:decision,p_note:note||null}:{p_approval:id,p_decision:decision,p_note:note||null};const r=await supabase.rpc(name,args);if(r.error)throw r.error;return r.data},
 ...
 async submitPayment(estimateId,status,amount,requestKey){const r=await supabase.rpc('submit_payment_for_approval',{p_estimate:estimateId,p_status:status,p_amount:Number(amount),p_request_key:requestKey});if(r.error)throw r.error;return r.data}
};
