...
const STAFF_SESSION_KEY='torvo_staff_session_id';
...
export async function forceSignOut(){if(!supabase)return;try{const client=requireBackend();const sid=localStorage.getItem(STAFF_SESSION_KEY);if(sid)await client.rpc('staff_revoke_my_sessions')}catch{}finally{localStorage.removeItem(STAFF_SESSION_KEY)}const{error}=await supabase.auth.signOut();if(error)throw error}
export async function signOut(){if(!supabase)return;let user=null;try{user=await currentAppUser()}catch{}if(user?.active&&['owner','admin'].includes(user.role)){window.dispatchEvent(new CustomEvent('torvo:backup-close'));return}return forceSignOut()}
