...
export async function loadPublicAndroidUpdate(){const{data,error}=await requireBackend().rpc('public_android_app_update');if(error)throw error;if(data==null)return null;if(!Array.isArray(data))throw new Error('INVALID APP UPDATE RESPONSE');const release=cleanRelease(data[0]);return release&&isProductionAndroidRelease({...release,platform:'android_apk',status:'published',production_signed:true})&&/^[0-9a-f]{64}$/.test(sha256Of(release))?release:null}
