...
export async function createAdditionalPO({parentOrderId,items,note}){const parent=id(parentOrderId,'PARENT ORDER');const orderId=await requestDealerAdditionalOrder(parent,lines(items),reason(note));return{order_id:orderId,parent_order_id:parent,status:'PENDING_APPROVAL'}}
...
export const createApprovedAddOnOrder=({requestId,items})=>createApprovedAdditionalOrder(id(requestId,'ADD-ON REQUEST'),lines(items));
