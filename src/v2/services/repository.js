...
export const data={
 ...
 async dispatches(){const{data,error}=await db().from('dispatches').select('*,sales_documents(id,final_payable,dealer_id,dealers(shop_name,dealer_code))').order('status');fail(error);return data??[]},
