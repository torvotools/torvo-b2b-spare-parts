...
export const data={
 ...
 async saveCatalog(item){const{data,error}=await db().rpc('upsert_catalog_item',{p_item:item.id||null,p_type:item.item_type,p_code:item.item_code,p_name:item.name,p_brand:item.brand||null,p_category:item.category||null,p_model:item.model||null,p_image_url:item.image_url||null,p_gst_mode:item.gst_mode||null,p_active:item.active!==false});fail(error);return data},
 async setRate(itemId,rateGroup,minQty,rate){const{data,error}=await db().rpc('set_item_rate',{p_item:itemId,p_rate_group:rateGroup,p_min_qty:minQty,p_rate:rate});fail(error);return data},
 async setCompatibility(machineId,sparePartId,{requiredQty=1,fitmentType='compatible',dealerVisible=false,publicVisible=false,notes=null}={}){const{data,error}=await db().rpc('set_machine_spare_mapping',{p_machine:machineId,p_spare_part:sparePartId,p_required_qty:requiredQty,p_fitment_type:fitmentType,p_dealer_visible:dealerVisible,p_public_visible:publicVisible,p_notes:notes});fail(error);return data},
 async createQuery(dealerId,lines){const{data,error}=await db().rpc('create_sales_query',{p_dealer:dealerId,p_lines:lines});fail(error);return data},
 async createQuotation(queryId,lines){const{data,error}=await db().rpc('create_quotation',{p_query:queryId,p_lines:lines});fail(error);return data},
 async acceptQuotation(quotationId){const{error}=await db().rpc('accept_quotation',{p_quotation:quotationId});fail(error)},
 async createSalesOrder(quotationId){const{data,error}=await db().rpc('quotation_to_sales_order',{p_quotation:quotationId});fail(error);return data},
 async createEstimate(salesOrderId,freight=0,otherCharges=0){const{data,error}=await db().rpc('sales_order_to_estimate',{p_sales_order:salesOrderId,p_freight:freight,p_other_charges:otherCharges});fail(error);return data}
};
