...
export async function createCustomerProductDemand({name,mobile,pin,searchText,productId=null,brand=null,modelNumber=null,requirementNote=null,marketing=false}={}){
 const fullName=cleanText(name,120),m=cleanMobile(mobile),p=cleanPin(pin),q=cleanText(searchText,200),pid=cleanId(productId)||null,b=cleanText(brand,120)||null,model=cleanText(modelNumber,120)||null,note=cleanText(requirementNote,500)||null;
...
export async function loadCustomerProductDemandResult(demandId,mobile){
 ...
 const status=String(result.status||'').trim().toLowerCase();return{demand_id:result.demand_id||id,status,search_text:cleanText(result.search_text,200),message:cleanText(result.message,500),found_dealer_id:status==='available'?result.found_dealer_id||null:null,found_contact_note:status==='available'?cleanText(result.found_contact_note,1000)||null:null,available_at:status==='available'?result.available_at||null:null};
}
