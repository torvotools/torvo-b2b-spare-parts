...
const leadSource=v=>normalizeLeadSource(v)||detectLeadSource();
const UUID_RE=/^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
const requireUuid=(v,label='ID')=>{const id=cleanId(v);if(!id||!UUID_RE.test(id))throw new Error(`${label} IS INVALID`);return id};
...
export async function loadCustomerProductDemandResult(demandId,mobile){const id=requireUuid(demandId,'CUSTOMER REQUIREMENT'),m=cleanMobile(mobile);if(m.length!==10)throw new Error('10-DIGIT MOBILE REQUIRED');const{data,error}=await db().rpc('public_customer_demand_result',{p_demand_id:id,p_mobile:m});if(error)throw error;const result=one(data);if(!result)return null;const status=String(result.status||'').trim().toLowerCase();return{demand_id:result.demand_id||id,status,search_text:cleanText(result.search_text,200,'PRODUCT SEARCH'),message:cleanText(result.message,500,'MESSAGE'),found_dealer_id:status==='available'?result.found_dealer_id||null:null,found_contact_note:status==='available'?cleanText(result.found_contact_note,1000,'CONTACT NOTE')||null:null,available_at:status==='available'?result.available_at||null:null}}
