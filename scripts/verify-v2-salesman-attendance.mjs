...
const[sql,service,auth,order,login,attendance,workspace,app]=await Promise.all([readFile('supabase/v2-salesman-attendance.sql','utf8'),readFile('src/v2/services/salesmanAttendance.js','utf8'),readFile('src/v2/services/staffAuth.js','utf8'),readFile('supabase/V2_INSTALL_ORDER.md','utf8'),readFile('src/v2/components/BusinessLogin.jsx','utf8'),readFile('src/v2/components/SalesmanAttendance.jsx','utf8'),readFile('src/v2/components/SalesmanAppWorkspace.jsx','utf8'),readFile('src/v2/App.jsx','utf8')]);
const gates=[
 ['ATTENDANCE TABLE',sql.includes('create table if not exists public.salesman_attendance')],
 ['ONE DAILY RECORD',sql.includes('unique(salesman_user_id,attendance_date)')&&sql.includes('TODAY ATTENDANCE ALREADY CHECKED IN')],
 ...
 ['ADMIN REPORT OWNER ADMIN ONLY',sql.includes('admin_salesman_attendance')&&sql.includes("lower(role) in('owner','admin')")],
 ['ADMIN REPORT DATE RANGE',sql.includes("raise exception 'INVALID ATTENDANCE DATE RANGE'")],
 ['CLIENT ATTENDANCE SERVICE',service.includes('attendanceCheckIn')&&service.includes('attendanceCheckOut')&&service.includes('myAttendanceHistory')&&service.includes('adminSalesmanAttendance')],
 ...
 ['SALESMAN APP MOUNTED',app.includes("import SalesmanAppWorkspace")&&app.includes('<SalesmanAppWorkspace/>')],
