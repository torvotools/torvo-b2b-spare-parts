...
const[sql,service,auth,order]=await Promise.all([readFile('supabase/v2-salesman-attendance.sql','utf8'),readFile('src/v2/services/salesmanAttendance.js','utf8'),readFile('src/v2/services/staffAuth.js','utf8'),readFile('supabase/V2_INSTALL_ORDER.md','utf8')]);
const gates=[
 ...
 ['DEALER MOBILE MODE',auth.includes("mode==='dealer_mobile'")&&auth.includes('normalizeDealerMobile')],
 ['STAFF ID MODE',auth.includes("mode==='staff_id'")&&auth.includes('STAFF USER ID')],
