...
for(const k of ['identity_ref','role','approved_device_ref','one_time_password_consumed_once','password_replay_failed','unapproved_device_failed','runtime_role_boundary_passed','revoke_invalidated_session'])if(s[k]===undefined||s[k]===null||s[k]==='')fail('staff missing '+k);
if(!['SALESMAN','STORE KEEPER','ACCOUNTANT'].includes(String(s.role).toUpperCase()))fail('unsupported staff acceptance role');
for(const [scope,obj,keys] of [['dealer',d,['pin_login_succeeded','device_b_replaced_a','device_a_revoked_private_reads_failed','device_a_revoked_private_writes_failed','device_b_remained_valid','logout_or_revoke_failed_afterwards']],['staff',s,['one_time_password_consumed_once','password_replay_failed','unapproved_device_failed','runtime_role_boundary_passed','revoke_invalidated_session']]])for(const k of keys)if(obj[k]!==true)fail(scope+' '+k+' is not true');
