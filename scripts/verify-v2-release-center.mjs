...
const checks=[
 ...
 ['VERIFIED DOWNLOAD CREDENTIALS CLOSED',svc.includes('u.username||u.password')],
 ['VERIFIED DOWNLOAD MALFORMED URL CLOSED',svc.includes('catch{return null}')],
 ['RELEASE LOAD ARRAY BOUNDARY',svc.includes("if(data==null)return[]")&&svc.includes("throw new Error('INVALID APP RELEASE RESPONSE')")&&svc.includes('data.filter(cleanRelease)')],
 ...
 ['RELEASE UI CONNECTED',ui.includes('loadAppReleases')&&ui.includes('DOWNLOAD VERIFIED TEST FILE')&&ui.includes('DOWNLOAD LATEST UPDATE')],
 ['RELEASE UI TEST PRODUCTION SEPARATED',ui.includes('TEST APK FILES NEVER BECOME AUTOMATIC PRODUCTION UPDATES')&&ui.includes('LATEST PRODUCTION ANDROID UPDATE')],
 ['RELEASE UI PACKAGE CONTINUITY',ui.includes('TORVO_ANDROID_PACKAGE')&&ui.includes('VERIFIED PRODUCTION SIGNATURE')],
