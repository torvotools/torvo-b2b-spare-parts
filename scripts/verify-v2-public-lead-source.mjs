...
const sources=['WEBSITE','FACEBOOK','INSTAGRAM','YOUTUBE','WHATSAPP','EMAIL','OTHER'];
for(const v of sources){if(!source.includes(`'${v}'`)||!sql.includes(`'${v}'`))fail(`${v} SOURCE MISSING`)}
...
const tables=['customer_contacts','customer_dealer_referrals','customer_repair_requirements','customer_product_demands'];
for(const table of tables){if(!sql.includes(`alter table ${table} add column if not exists lead_source text`))fail(`${table} LEAD SOURCE COLUMN MISSING`);if(!sql.includes(`${table}_lead_source_check`))fail(`${table} LEAD SOURCE CHECK CONSTRAINT MISSING`);if(!sql.includes(`${table}_lead_source_idx`))fail(`${table} LEAD SOURCE REPORTING INDEX MISSING`)}
if(!sql.includes("lead_source is null or lead_source in('WEBSITE','FACEBOOK','INSTAGRAM','YOUTUBE','WHATSAPP','EMAIL','OTHER')"))fail('DATABASE LEAD SOURCE ALLOWLIST CONSTRAINT MISSING');
...
console.log('TORVO V2 public + customer app lead source persistence contract OK');
