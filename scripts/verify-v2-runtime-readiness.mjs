...
const gates=read('docs/TORVO-V2-RELEASE-GATES.md'),install=read('supabase/V2_INSTALL_ORDER.md'),auth=read('supabase/functions/_shared/torvo-auth.ts'),login=read('supabase/functions/dealer-email-otp/index.ts'),staffEmail=read('supabase/functions/staff-email-otp/index.ts'),release=read('supabase/v2-app-release-center.sql'),accountant=read('supabase/v2-accountant-workspace-buttons.sql'),accountantClient=read('src/v2/services/accountantWorkspace.js');
const checks=[
 ...
 ['DEALER LOGIN EDGE FUNCTION PRESENT',login.includes("rpc('dealer_email_otp_begin'")&&login.includes("rpc('dealer_email_otp_verify'")&&login.includes("rpc('dealer_start_device_session'")&&login.includes('establishSession')],
