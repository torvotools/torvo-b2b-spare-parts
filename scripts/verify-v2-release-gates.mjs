...
const checks=[
 ...
 ['EXACT SHA PREVIEW REQUIRED',gates.includes('EXACT RELEASE SHA MUST PASS BUILD CHECK AND CLOUDFLARE PREVIEW')],
 ...
 ['RUNTIME VERIFY IN BUILD',String(pkg.scripts['verify:v2-build']).includes('verify:v2-runtime')],
 ['APP BUILD USES RELEASE GATES',String(pkg.scripts['build:app']).includes('verify:v2-release-gates')&&String(pkg.scripts['build:app']).includes('verify:v2-runtime')]
];
