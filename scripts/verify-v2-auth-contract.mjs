...
const pin=read('supabase/v2-dealer-pin-auth.sql'),missing=read('supabase/v2-dealer-missing-part-request.sql'),staff=read('src/v2/services/staffAuth.js'),session=read('src/v2/services/dealerSession.js'),auth=read('src/v2/services/auth.js'),app=read('src/v2/App.jsx');
const checks=[
 ...
 ['DEALER CLIENT REQUIRES OPAQUE TOKEN',staff.includes("if(!data?.ok||!data?.dealer_session_token)")&&staff.includes('DEALER_SESSION_KEY')],
 ['FAILED LOGIN CLEARS OLD DEALER TOKEN',staff.includes('clearDealerSession();const{data,error}=await client.functions.invoke')],
 ['DEVICE ID MUST BE SECURE',staff.includes('crypto?.randomUUID')&&staff.includes('assertDevice(id)')],
