...
const requirementStatus="status text not null default 'OPEN' check(status in('OPEN','UNDER REVIEW','PRODUCT ADDED','FULFILLED','CLOSED'))";
if(!supportSql.includes(requirementStatus))fail('AUTHORITATIVE PRODUCT REQUIREMENT STATUS CONTRACT MISSING');
const requirementTable=supportSql.match(/create table if not exists public\.customer_product_requirements\(([\s\S]*?)\n\);/)?.[1]||'';
if(requirementTable.includes("'RESOLVED'")||requirementTable.includes("'REJECTED'"))fail('COMPLAINT-ONLY STATUS LEAKED INTO PRODUCT REQUIREMENTS');
