...
for(const token of["import PublicLocationDropdowns from'./PublicLocationDropdowns'",'<PublicLocationDropdowns value={form} onChange={setForm} disabled={busy}/>','!form.state||!form.district||!form.city','registerDealer({...form,mobile,pin})','PENDING VERIFICATION'])if(!dealerUi.includes(token))fail(`DEALER REGISTRATION LOCATION WIRING MISSING: ${token}`);
