...
for(const token of["const same=(a,b)=>String(a||'').trim().toUpperCase()===String(b||'').trim().toUpperCase()","const name=String(value?.state||'').trim()","states.find(x=>same(x.name,name))","loadLocationDistricts(row.id)","const name=String(value?.district||'').trim()","districts.find(x=>same(x.name,name))","loadLocationCities(row.id)","const cityId=cities.find(x=>same(x.name,value?.city))?.id||''"])if(!ui.includes(token))fail(`SAVED LOCATION HYDRATION MISSING: ${token}`);
