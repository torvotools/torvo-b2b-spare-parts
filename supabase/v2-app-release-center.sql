...
alter table app_release_artifacts drop constraint if exists app_release_artifacts_min_not_ahead_check;
alter table app_release_artifacts add constraint app_release_artifacts_min_not_ahead_check check(min_supported_build is null or min_supported_build<=build_number);
alter table app_release_artifacts drop constraint if exists app_release_artifacts_published_contract_check;
alter table app_release_artifacts add constraint app_release_artifacts_published_contract_check check(status<>'published' or (artifact_url is not null and artifact_url~'^https://' and artifact_sha256~'^[0-9a-f]{64}$' and published_at is not null));
alter table app_release_artifacts drop constraint if exists app_release_artifacts_android_production_check;
alter table app_release_artifacts add constraint app_release_artifacts_android_production_check check(platform<>'android_apk' or status<>'published' or (production_signed=true and package_id='com.torvotools.app' and channel in('production','play','stable')));
