...
begin
 ...
 update staff_email_otp_challenges set revoked_at=now() where consumed_at is null and revoked_at is null;
...
grant execute on function staff_email_otp_verify(uuid,text,text,text) to service_role; or provided_email<>e or length(d)<8 or length(d)>180 or p_otp !~ '^[0-9]{6}$' then raise exception 'INVALID LOGIN'; end if;
 select sai.* into i from staff_access_identities sai join app_users a on a.id=sai.app_user_id
 where upper(sai.username)=upper(btrim(coalesce(p_username,''))) and sai.active=true and a.active=true
 and lower(coalesce(a.role,'')) in('admin','accountant','salesman','store_keeper') and lower(coalesce(sai.staff_role,''))=lower(a.role)
 for update of sai;
