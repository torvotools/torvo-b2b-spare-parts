...
-- REUSES dealer_discovery_leads + customer_service_dealer_offers. DOES NOT CREATE APPROVED DEALERS.
-- RUN AFTER v2-referral-repair-routing.sql AND EXISTING SALESMAN TERRITORY MODULE.
...
declare u app_users%rowtype;v_id uuid;v_mobile text;v_cap text;
begin
 ...
 if nullif(btrim(p_shop_name),'') is null then raise exception 'SHOP NAME REQUIRED';end if;
 ...
 -- Never create a second lead when an approved/existing Dealer already uses this mobile.
 if exists(select 1 from dealers d where regexp_replace(coalesce(d.mobile,''),'\D','','g')=v_mobile) then raise exception 'DEALER ALREADY EXISTS FOR THIS MOBILE';end if;
 insert into dealer_discovery_leads(shop_name,contact_person,mobile,whatsapp,pin_code,area,capability,source,submitted_by,status)
 values(upper(btrim(p_shop_name)),nullif(upper(btrim(p_contact_person)),''),v_mobile,v_mobile,btrim(p_pin_code),nullif(upper(btrim(p_area)),''),v_cap,'salesman_field',u.id,'new')
 on conflict(mobile) where status in('new','verification_pending') do update set shop_name=excluded.shop_name,contact_person=excluded.contact_person,pin_code=excluded.pin_code,area=excluded.area,capability=excluded.capability,updated_at=now()
...
revoke all on function salesman_submit_dealer_discovery_lead(text,text,text,text,text,text) from public;grant execute on function salesman_submit_dealer_discovery_lead(text,text,text,text,text,text) to authenticated;
...
language sql security definer set search_path=public as $$
 select l.id,l.shop_name,l.contact_person,l.mobile,l.pin_code,l.area,l.capability,l.status,l.created_at from dealer_discovery_leads l join app_users u on u.id=l.submitted_by where u.auth_user_id=auth.uid() and u.active=true and u.role='salesman' order by l.created_at desc limit 100
$$;revoke all on function salesman_my_dealer_discovery_leads() from public;grant execute on function salesman_my_dealer_discovery_leads() to authenticated;

-- Salesman sees open repair requirements without customer contact details.
...
revoke all on function salesman_open_repair_requirements() from public;grant execute on function salesman_open_repair_requirements() to authenticated;

create or replace function salesman_offer_repair_to_dealer(p_requirement_id uuid,p_dealer_id uuid)
returns uuid language plpgsql security definer set search_path=public as $$
declare u app_users%rowtype;v_id uuid;
begin
 ...
 update customer_service_requirements set status='dealers_offered',updated_at=now() where id=p_requirement_id and status='received';
 insert into audit_log(actor_id,action,entity_type,entity_id,details) values(u.id,'REPAIR_REQUIREMENT_OFFERED','customer_service_requirement',p_requirement_id::text,jsonb_build_object('dealer_id',p_dealer_id,'offer_id',v_id));return v_id;
end$$;
revoke all on function salesman_offer_repair_to_dealer(uuid,uuid) from public;grant execute on function salesman_offer_repair_to_dealer(uuid,uuid) to authenticated;
