...
-- Keep repair requirements separate from retail/accounting. No Dealer base selling/labour price is stored.
create table if not exists customer_service_requirements(
 ...
 intent text not null default 'repair_service' check(intent in('repair_service')),
 brand text,
 model_number text,
 ...
 assigned_dealer_id uuid references dealers(id) on delete restrict,
 referral_id uuid references customer_dealer_referrals(id) on delete restrict,
...
-- Private media metadata. Store object paths only; bucket/object access must remain private and use signed/authorized URLs.
create table if not exists customer_requirement_media(
 id uuid primary key default gen_random_uuid(),
 requirement_id uuid not null references customer_service_requirements(id) on delete cascade,
...
-- One offer row per requirement/dealer prevents duplicate broadcasts and preserves Dealer-specific response history.
create table if not exists customer_service_dealer_offers(
 ...
 offered_by uuid not null references app_users(id),
...
-- Useful field-discovery lead only. Salesman can submit a Retailer/Repair Dealer lead; approval remains TORVO controlled.
create table if not exists dealer_discovery_leads(
 ...
 area text,
 capability text not null default 'product_sales' check(capability in('product_sales','repair_service','both')),
 source text not null default 'salesman_field',
 submitted_by uuid not null references app_users(id),
 status text not null default 'new' check(status in('new','duplicate','verification_pending','converted','rejected')),
 converted_dealer_id uuid references dealers(id) on delete restrict,
 created_at timestamptz not null default now(),
 updated_at timestamptz not null default now()
);
...
-- Exact PIN is preferred. If none exists, return other verified Dealers instead of pretending a fake distance.
-- Without real coordinates this fallback is explicitly labelled OTHER_AREA; UI may show PIN/area, never fake kilometres.
...
language sql security definer set search_path=public as $$
 with eligible as(
  ...
  select e.*,case when e.effective_pin=btrim(p_pin_code) then 0 else 1 end match_rank
  from eligible e
 )
 select r.id,r.shop_name,coalesce(nullif(r.public_address,''),r.address),r.effective_pin,r.map_url,r.latitude,r.longitude,r.product_sales_available,r.repair_service_available,r.authorized_service_center,r.authorized_service_note,
        case when r.match_rank=0 then 'EXACT_PIN' else 'OTHER_AREA' end
 from ranked r
 order by r.match_rank,r.shop_name
...
-- Dealer response: reveal no Customer contact here. Contact release happens only after TORVO assignment.
...
begin
 ...
 select d.* into v_dealer from dealers d where d.mobile=v_user.mobile and d.status='approved' and d.repair_service_available=true;
 ...
 if v_offer.status in('assigned','withdrawn') then raise exception 'OFFER IS CLOSED';end if;
 update customer_service_dealer_offers set status=v_status,dealer_note=nullif(btrim(p_note),''),responded_at=coalesce(responded_at,now()) where id=v_offer.id;
 return v_offer.id;
end$$;
revoke all on function dealer_respond_service_offer(uuid,text,text) from public;
grant execute on function dealer_respond_service_offer(uuid,text,text) to authenticated;

alter table customer_service_requirements enable row level security;
alter table customer_requirement_media enable row level security;
alter table customer_service_dealer_offers enable row level security;
alter table dealer_discovery_leads enable row level security;
