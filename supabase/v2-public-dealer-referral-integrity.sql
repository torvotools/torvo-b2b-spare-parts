...
returns boolean language plpgsql security definer set search_path=public as $$declare e text:=upper(btrim(coalesce(p_event_type,'')));begin
 ...
 if p_product_id is not null and not exists(select 1 from catalog_items c where c.id=p_product_id and coalesce(c.is_active,true)=true) then raise exception 'PRODUCT NOT AVAILABLE'; end if;
...
language plpgsql security definer set search_path=public as $$declare m text:=right(regexp_replace(coalesce(p_mobile,''),'\D','','g'),10);p text:=btrim(coalesce(p_pin_code,''));src text:=torvo_normalize_public_lead_source(p_lead_source);c customer_contacts%rowtype;b referral_benefit_campaigns%rowtype;r customer_dealer_referrals%rowtype;qid uuid;qno bigint;begin
 ...
 if not exists(select 1 from catalog_items i where i.id=p_product_id and coalesce(i.is_active,true)=true) then raise exception 'ACTIVE PRODUCT REQUIRED'; end if;if p_dealer_id is null then raise exception 'SELECT VERIFIED DEALER REQUIRED'; end if;
 ...
 return query select r.id,r.referral_code,b.benefit_type,b.benefit_value,b.benefit_text,r.expires_at,qid;
end$$;
