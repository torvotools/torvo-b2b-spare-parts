...
do $$begin if to_regclass('public.dealer_referral_events') is not null then alter table dealer_referral_events drop constraint if exists dealer_referral_events_event_type_check;alter table dealer_referral_events add constraint dealer_referral_events_event_type_check check(event_type in('DEALER_VIEW','MAP_OPEN','CALL_CLICK','WHATSAPP_CLICK','REFERRAL_CREATED','CONFIRMED_CONVERSION'));end if;end$$;
...
language plpgsql security definer set search_path=public as $$declare m text:=right(regexp_replace(coalesce(p_mobile,''),'\D','','g'),10);p text:=btrim(coalesce(p_pin_code,''));src text:=torvo_normalize_public_lead_source(p_lead_source);c customer_contacts%rowtype;b referral_benefit_campaigns%rowtype;r customer_dealer_referrals%rowtype;qid uuid;qno bigint;begin
 if length(btrim(coalesce(p_full_name,'')))<2 then raise exception 'CUSTOMER NAME REQUIRED'; end if;if m !~ '^[0-9]{10}$' then raise exception '10-DIGIT MOBILE / WHATSAPP REQUIRED'; end if;if p !~ '^[0-9]{6}$' then raise exception '6-DIGIT PIN CODE REQUIRED'; end if;
 if not exists(select 1 from catalog_items i where i.id=p_product_id and coalesce(i.active,true)=true) then raise exception 'ACTIVE PRODUCT REQUIRED'; end if;if p_dealer_id is null then raise exception 'SELECT VERIFIED DEALER REQUIRED'; end if;
 if not exists(select 1 from dealers d where d.id=p_dealer_id and lower(coalesce(d.status,''))='approved' and d.customer_referral_enabled=true and d.referral_profile_verified_at is not null and d.product_sales_available=true and coalesce(nullif(d.public_pin_code,''),d.pin_code)=p) then raise exception 'SELECTED DEALER DOES NOT HAVE VERIFIED COVERAGE FOR THIS PIN'; end if;
 insert into customer_contacts(full_name,mobile,whatsapp,pin_code,marketing_opt_in,marketing_opt_in_at,marketing_opt_in_source,marketing_opt_out_at,consent_updated_at,lead_source,updated_at) values(upper(btrim(p_full_name)),m,m,p,p_marketing_opt_in,case when p_marketing_opt_in then now() end,src,case when not p_marketing_opt_in then now() end,now(),src,now()) on conflict(mobile) do update set full_name=excluded.full_name,whatsapp=excluded.whatsapp,pin_code=excluded.pin_code,marketing_opt_in=excluded.marketing_opt_in,marketing_opt_in_at=case when excluded.marketing_opt_in then coalesce(customer_contacts.marketing_opt_in_at,now()) else null end,marketing_opt_in_source=src,marketing_opt_out_at=case when excluded.marketing_opt_in then null else now() end,consent_updated_at=now(),lead_source=src,updated_at=now() returning * into c;
 ...
 insert into customer_product_enquiries(customer_name,mobile_whatsapp,pin_code,marketing_opt_in,marketing_consent_at,marketing_consent_source,marketing_opted_out_at,status) values(upper(btrim(p_full_name)),m,p,p_marketing_opt_in,case when p_marketing_opt_in then now()end,src,case when not p_marketing_opt_in then now()end,'DEALER_REFERRED') returning id,enquiry_no into qid,qno;
 insert into customer_product_enquiry_items(enquiry_id,product_id) values(qid,p_product_id) on conflict do nothing;
 insert into customer_dealer_referrals(referral_code,customer_id,product_id,dealer_id,benefit_campaign_id,customer_pin_code,status,expires_at,dealer_selected_at,lead_source) values('TRV-'||upper(substr(encode(gen_random_bytes(6),'hex'),1,10)),c.id,p_product_id,p_dealer_id,b.id,p,'dealer_selected',now()+interval'30 days',now(),src) returning*into r;
 insert into dealer_referral_events(enquiry_id,dealer_id,event_type,product_id) values(qid,p_dealer_id,'REFERRAL_CREATED',p_product_id);
 return query select r.id,r.referral_code,b.benefit_type,b.benefit_value,b.benefit_text,r.expires_at,qid;end$$;
revoke all on function public_create_customer_referral(text,text,text,uuid,uuid,boolean,text) from public;grant execute on function public_create_customer_referral(text,text,text,uuid,uuid,boolean,text) to anon,authenticated;
