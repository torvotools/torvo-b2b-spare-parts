...
-- Lock demand + dealer so close/cancel or dealer suspension cannot race a new assignment.
...
declare u app_users%rowtype;d customer_product_demands%rowtype;target_dealer dealers%rowtype;rid uuid;stage text:=upper(btrim(coalesce(p_routing_stage,'')));
begin
...
language plpgsql security definer set search_path=public as $$declare did uuid;l customer_demand_dealer_leads%rowtype;d customer_product_demands%rowtype;au uuid;identity_count integer;
begin
 did:=dealer_assert_my_device_session(p_device_id,p_session_token);
 ...
 select id into strict au from app_users where auth_user_id=auth.uid() and active=true and lower(coalesce(role,''))='dealer' and dealer_id=did;
