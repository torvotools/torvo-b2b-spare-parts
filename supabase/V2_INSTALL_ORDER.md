...
7. RETURNS BASE after Delivery + received Purchase integrity. Install `v2-sales-purchase-returns.sql` only after delivered Sales and received Purchase foundations exist. Its final direct Sales/Purchase return completion RPCs must keep source-row serialization so concurrent returns cannot validate against the same pre-return quantity.
8. CENTRAL MAKER-CHECKER and final approval boundaries. After the return approval gate exists, install `v2-return-approval-checker-fix.sql`; this is the final approved-return checker implementation and must keep the same source-row serialization before final quantity revalidation. Do not restore an older return approval/completion definition after these final hardening files.
