...
11. CUSTOMER/DEALER NETWORK BASE and public referral/repair/registration/service-area/support foundations. After `v2-customer-dealer-referral-network.sql` install `v2-customer-marketing-consent-segmentation.sql`. After `v2-admin-managed-experience.sql`, install `v2-customer-public-runtime-contract.sql` so the public CustomerApp locator/referral/settings RPC names exactly match the runtime client contract. Marketing profile data is voluntary, consent-audited and private. After `v2-public-dealer-registration.sql`, install `v2-accountant-dealer-verification.sql`, then after `v2-business-rpcs.sql` install `v2-dealer-final-approval-accountant-gate.sql`.
