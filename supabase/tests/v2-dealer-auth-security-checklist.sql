...
select to_regprocedure('public.get_dealer_machine_spares(uuid)') legacy_machine,to_regprocedure('public.dealer_knowledge_challenges()') legacy_challenges,to_regprocedure('public.dealer_knowledge_history()') legacy_history,to_regprocedure('public.submit_knowledge_answer(uuid,text,text,text,text,text)') legacy_knowledge,to_regprocedure('public.dealer_confirm_sales_order_revision(uuid,integer)') legacy_confirm,to_regprocedure('public.request_additional_purchase_order(uuid,jsonb,text)') legacy_addon,to_regprocedure('public.get_dealer_order_history_30d()') legacy_history_order,to_regprocedure('public.dealer_verify_customer_referral(text)') legacy_verify,to_regprocedure('public.dealer_order_referral_item(text,numeric)') legacy_referral_order;
...
-- 4. REVOKED DEVICE A MUST FAIL MISSING-PART, CATALOG, MACHINE-SPARES, FITMENT AND REFERRAL ACTIONS.
-- 5. REVOKED DEVICE A MUST FAIL SALES-ORDER CONFIRMATION, ADDITIONAL PURCHASE ORDER REQUEST AND 30-DAY ORDER HISTORY EVEN IF SUPABASE AUTH TOKEN REMAINS VALID.
-- 6. DEVICE B REMAINS VALID AND EVERY PROTECTED DEALER RPC DERIVES DEALER SERVER-SIDE.
-- 7. ADDITIONAL ORDER QUANTITY MUST BE INTEGER 1..9999 AND DUPLICATE ITEM LINES MUST FAIL.
