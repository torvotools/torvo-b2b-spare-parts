...
select p.proname,pg_get_function_identity_arguments(p.oid) args,has_function_privilege('public',p.oid,'EXECUTE') public_exec,has_function_privilege('anon',p.oid,'EXECUTE') anon_exec,has_function_privilege('authenticated',p.oid,'EXECUTE') authenticated_exec from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname in('dealer_assert_my_device_session','dealer_workspace_catalog','dealer_item_rate','submit_purchase_order','get_dealer_machine_spares','dealer_knowledge_challenges','dealer_knowledge_history','submit_knowledge_answer','dealer_verify_customer_referral','dealer_confirm_referral_benefit','dealer_referral_supply_status','dealer_order_referral_item','dealer_confirm_sales_order_revision','request_additional_purchase_order','get_dealer_order_history_30d') order by p.proname,args;
...
-- 4. REVOKED DEVICE A MUST FAIL PRIVATE WORKSPACE CATALOG, DEALER RATE LOOKUP AND PURCHASE ORDER SUBMISSION EVEN IF ITS SUPABASE AUTH TOKEN REMAINS VALID.
