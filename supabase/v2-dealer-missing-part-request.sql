...
declare v_user app_users%rowtype;v_dealer dealers%rowtype;v_id uuid;v_mobile text;v_brand text:=nullif(upper(btrim(coalesce(p_machine_brand,''))),'');v_model text:=nullif(upper(btrim(coalesce(p_machine_model,''))),'');v_part text:=nullif(upper(btrim(coalesce(p_part_name,''))),'');v_message text:=nullif(btrim(coalesce(p_dealer_message,'')),'');v_photo text:=nullif(btrim(coalesce(p_photo_url,'')),'');
begin
 ...
 select d.* into v_dealer from dealers d where right(regexp_replace(coalesce(d.mobile,''),'\D','','g'),10)=v_mobile and lower(coalesce(d.status,''))='approved' order by d.created_at desc limit 1;
 if not found then raise exception 'APPROVED DEALER LINK REQUIRED';end if;
...
declare v_user app_users%rowtype;v_dealer dealers%rowtype;v_mobile text;
begin
 if auth.uid() is null then raise exception 'AUTHENTICATION REQUIRED';end if;
 select * into v_user from app_users where auth_user_id=auth.uid() and active=true limit 1;
 if not found or lower(v_user.role)<>'dealer' then raise exception 'ACTIVE DEALER LOGIN REQUIRED';end if;
