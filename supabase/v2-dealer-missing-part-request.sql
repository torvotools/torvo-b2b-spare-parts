...
create or replace function public.dealer_create_missing_part_request(
  p_machine_brand text default null,p_machine_model text default null,p_part_name text default null,p_requested_qty numeric default null,p_dealer_message text default null,p_photo_url text default null
) returns uuid language plpgsql security definer set search_path=public as $$
declare v_user app_users%rowtype;v_dealer dealers%rowtype;v_id uuid;v_brand text:=nullif(upper(btrim(coalesce(p_machine_brand,''))),'');v_model text:=nullif(upper(btrim(coalesce(p_machine_model,''))),'');v_part text:=nullif(upper(btrim(coalesce(p_part_name,''))),'');v_message text:=nullif(btrim(coalesce(p_dealer_message,'')),'');v_photo text:=nullif(btrim(coalesce(p_photo_url,'')),'');
begin
  ...
  select d.* into v_dealer from dealers d where regexp_replace(coalesce(d.mobile,''),'\D','','g')=regexp_replace(coalesce(v_user.mobile,''),'\D','','g') order by d.created_at desc limit 1;
  if not found or lower(coalesce(v_dealer.status,''))<>'approved' then raise exception 'APPROVED DEALER LINK REQUIRED'; end if;
  if v_part is null and v_message is null then raise exception 'PART NAME OR REQUIREMENT DESCRIPTION REQUIRED'; end if;
  if length(coalesce(v_brand,''))>80 then raise exception 'MACHINE BRAND IS TOO LONG'; end if;
  if length(coalesce(v_model,''))>120 then raise exception 'MACHINE MODEL IS TOO LONG'; end if;
  if length(coalesce(v_part,''))>160 then raise exception 'PART NAME IS TOO LONG'; end if;
  if length(coalesce(v_message,''))>1000 then raise exception 'REQUIREMENT DESCRIPTION IS TOO LONG'; end if;
  ...
  insert into missing_part_requests(dealer_id,photo_url,machine_brand,machine_model,part_name,requested_qty,dealer_message,status) values(v_dealer.id,'',v_brand,v_model,v_part,p_requested_qty,v_message,'new') returning id into v_id;
  return v_id;
end;$$;
