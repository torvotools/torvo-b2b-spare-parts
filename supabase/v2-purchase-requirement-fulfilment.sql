...
-- Purchase Rate is never returned here. Supplier identity is Owner-only; Admin receives operational invoice/date/quantity only.
...
create table if not exists purchase_requirement_link_dealer_allocations(
 id uuid primary key default gen_random_uuid(),link_id uuid not null references purchase_requirement_links(id) on delete restrict,requirement_dealer_id uuid not null references purchase_requirement_dealers(id) on delete restrict,allocated_qty numeric not null check(allocated_qty>0),created_at timestamptz not null default now(),reversed_at timestamptz,unique(link_id,requirement_dealer_id));
...
alter table purchase_requirement_link_dealer_allocations enable row level security;revoke all on purchase_requirement_link_dealer_allocations from anon,authenticated;
...
declare a app_users%rowtype;begin
 select * into a from app_users where auth_user_id=auth.uid() and active=true;if not found or a.role not in('owner','admin') then raise exception 'Owner/Admin required';end if;if not exists(select 1 from purchase_requirements where purchase_requirements.id=p_requirement) then raise exception 'Requirement not found';end if;
 return query select l.id,l.purchase_id,h.invoice_no,h.invoice_date,case when a.role='owner' then s.supplier_name else null end,l.linked_qty,l.item_id,l.note,l.linked_at,l.reversed_at from purchase_requirement_links l join purchase_headers h on h.id=l.purchase_id left join suppliers s on s.id=h.supplier_id where l.requirement_id=p_requirement order by l.linked_at desc;
end;$$;
...
declare a app_users%rowtype;r purchase_requirements%rowtype;approved numeric;already numeric;remaining numeric;link_item uuid;purchase_qty numeric;new_link uuid;alloc_left numeric;d record;give_qty numeric;begin
 select * into a from app_users where auth_user_id=auth.uid() and active=true;if not found or a.role not in('owner','admin') then raise exception 'Owner/Admin required';end if;if p_qty is null or p_qty<=0 then raise exception 'Valid linked quantity required';end if;
 select * into r from purchase_requirements where id=p_requirement for update;if not found then raise exception 'Requirement not found';end if;if r.status not in('approved','purchasing','partially_purchased') then raise exception 'Requirement is not open for purchase fulfilment';end if;
 ...
 perform 1 from purchase_headers where id=p_purchase for update;if not found then raise exception 'Purchase not found';end if;if exists(select 1 from audit_log where entity_type='purchase' and entity_id=p_purchase::text and action='PURCHASE_REVERSED') then raise exception 'Reversed Purchase cannot fulfil a requirement';end if;
 select coalesce(sum(qty),0) into purchase_qty from purchase_lines where purchase_id=p_purchase and item_id=link_item;if purchase_qty<=0 then raise exception 'Selected purchase does not contain the requirement item';end if;
 select coalesce(sum(linked_qty),0) into already from purchase_requirement_links where purchase_id=p_purchase and item_id=link_item and reversed_at is null;if already+p_qty>purchase_qty then raise exception 'Linked quantity exceeds purchased quantity for this item';end if;
 approved:=coalesce(r.approved_qty,r.requested_qty);remaining:=greatest(approved-r.purchased_qty,0);if remaining<=0 then raise exception 'Requirement is already fully purchased';end if;if p_qty>remaining then raise exception 'Linked quantity exceeds remaining approved requirement';end if;if exists(select 1 from purchase_requirement_links where requirement_id=r.id and purchase_id=p_purchase and reversed_at is null) then raise exception 'This purchase is already linked to the requirement';end if;
 insert into purchase_requirement_links(requirement_id,purchase_id,linked_by,linked_qty,item_id,note) values(r.id,p_purchase,a.id,p_qty,link_item,nullif(trim(p_note),'')) returning id into new_link;update purchase_requirements set purchased_qty=purchased_qty+p_qty,status=case when purchased_qty+p_qty>=approved then 'completed' else 'partially_purchased' end where id=r.id;
 alloc_left:=p_qty;for d in select id,greatest(coalesce(requested_qty,0)-fulfilled_qty,0) need from purchase_requirement_dealers where requirement_id=r.id and greatest(coalesce(requested_qty,0)-fulfilled_qty,0)>0 order by id for update loop exit when alloc_left<=0;give_qty:=least(d.need,alloc_left);if give_qty>0 then update purchase_requirement_dealers set fulfilled_qty=fulfilled_qty+give_qty,stock_arrival_recorded_at=now() where id=d.id;insert into purchase_requirement_link_dealer_allocations(link_id,requirement_dealer_id,allocated_qty) values(new_link,d.id,give_qty);alloc_left:=alloc_left-give_qty;end if;end loop;
...
declare a app_users%rowtype;l purchase_requirement_links%rowtype;r purchase_requirements%rowtype;approved numeric;x record;legacy_undo numeric;d record;take_qty numeric;begin
 ...
 if exists(select 1 from purchase_requirement_link_dealer_allocations where link_id=l.id and reversed_at is null) then for x in select a2.id,a2.requirement_dealer_id,a2.allocated_qty from purchase_requirement_link_dealer_allocations a2 where a2.link_id=l.id and a2.reversed_at is null order by a2.created_at desc,a2.id desc for update loop perform 1 from purchase_requirement_dealers where id=x.requirement_dealer_id for update;update purchase_requirement_dealers set fulfilled_qty=greatest(fulfilled_qty-x.allocated_qty,0),stock_arrival_recorded_at=case when greatest(fulfilled_qty-x.allocated_qty,0)=0 then null else stock_arrival_recorded_at end where id=x.requirement_dealer_id;update purchase_requirement_link_dealer_allocations set reversed_at=now() where id=x.id;end loop;
 else legacy_undo:=l.linked_qty;for d in select id,fulfilled_qty from purchase_requirement_dealers where requirement_id=r.id and fulfilled_qty>0 order by stock_arrival_recorded_at desc nulls last,id desc for update loop exit when legacy_undo<=0;take_qty:=least(d.fulfilled_qty,legacy_undo);update purchase_requirement_dealers set fulfilled_qty=greatest(fulfilled_qty-take_qty,0),stock_arrival_recorded_at=case when greatest(fulfilled_qty-take_qty,0)=0 then null else stock_arrival_recorded_at end where id=d.id;legacy_undo:=legacy_undo-take_qty;end loop;end if;
 update purchase_requirement_links set reversed_at=now(),reversed_by=a.id,note=concat_ws(' | ',note,'REVERSED: '||trim(p_reason)) where id=l.id;update purchase_requirements set purchased_qty=greatest(purchased_qty-l.linked_qty,0),status=case when greatest(purchased_qty-l.linked_qty,0)<=0 then 'purchasing' when greatest(purchased_qty-l.linked_qty,0)>=approved then 'completed' else 'partially_purchased' end where id=r.id;
