-- TORVO V2 secure reward accounting.
-- Requires v2-reward-lots.sql. Ledger is audit history; lots are spendable balances.

create or replace function reward_available_balance(p_dealer uuid)
returns numeric language sql stable security definer set search_path=public as $$
  select greatest(coalesce(sum(remaining_points),0),0)
  from reward_point_lots
  where dealer_id=p_dealer and remaining_points>0
...
declare a app_users%rowtype;v uuid;begin
 ...
 values(p_dealer,v,p_points,p_points,p_expires_at,now());
...
returns uuid language plpgsql security definer set search_path=public as $$
declare a app_users%rowtype;v uuid;bal numeric;need numeric;take numeric;r reward_point_lots%rowtype;begin
 select * into a from app_users where auth_user_id=auth.uid() and active=true;
 if not found or a.role not in('owner','admin') then raise exception 'Not authorized';end if;
 if p_points is null or p_points<=0 then raise exception 'Points must be greater than zero';end if;
 if nullif(trim(p_reason),'') is null then raise exception 'Reason required';end if;
 ...
 need:=p_points;
 for r in
   select * from reward_point_lots
   where dealer_id=p_dealer and remaining_points>0 and(expires_at is null or expires_at>now())
   order by expires_at asc nulls last,created_at,id for update
 loop
   exit when need<=0;
   take:=least(r.remaining_points,need);
   update reward_point_lots set remaining_points=remaining_points-take where id=r.id;
   insert into reward_redemption_allocations(redemption_entry_id,lot_id,points) values(v,r.id,take);
   need:=need-take;
 end loop;
 if need<>0 then raise exception 'Reward allocation failed';end if;
...
declare a app_users%rowtype;r reward_point_lots%rowtype;v_count integer:=0;v uuid;begin
 select * into a from app_users where auth_user_id=auth.uid() and active=true;
 if not found or a.role not in('owner','admin') then raise exception 'Not authorized';end if;
 for r in
   select * from reward_point_lots
   where remaining_points>0 and expires_at is not null and expires_at<=now()
     and(p_dealer is null or dealer_id=p_dealer)
   order by dealer_id,expires_at,created_at for update
 loop
   perform pg_advisory_xact_lock(hashtextextended(r.dealer_id::text,0));
   insert into reward_ledger(dealer_id,points,reason,entry_type,source_type,source_id,created_by)
   values(r.dealer_id,-r.remaining_points,'Expired reward points','expire','reward_lot',r.id::text,a.id)
   returning id into v;
   update reward_point_lots set remaining_points=0,expired_at=now() where id=r.id;
   insert into audit_log(actor_id,action,entity_type,entity_id,details)
   values(a.id,'REWARD_POINTS_EXPIRED','reward_ledger',v::text,jsonb_build_object('dealer_id',r.dealer_id,'lot_id',r.id,'points',r.remaining_points));
   v_count:=v_count+1;
 end loop;
