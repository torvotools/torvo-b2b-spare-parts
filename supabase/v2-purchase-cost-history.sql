-- TORVO V2 purchase-cost history. Owner-only financial source for future profit reports.
...
create or replace function get_profit_summary() returns jsonb language plpgsql stable security definer set search_path=public as $$
declare a app_users%rowtype;rev numeric:=0;cost numeric:=0;missing bigint:=0;begin
 select * into a from app_users where auth_user_id=auth.uid() and active=true;
 if not found or a.role<>'owner' then raise exception 'Owner authorization required';end if;
 with delivered as(
  select e.id,e.final_payable from sales_documents e join dispatches d on d.estimate_id=e.id where e.doc_type='estimate' and d.status='delivered'
 ),line_cost as(
  ...
  from sales_document_lines l join sales_documents e on e.id=l.document_id join delivered d on d.id=e.id
 )
 ...
 return jsonb_build_object('revenue',rev,'cost',cost,'profit',case when missing=0 then rev-cost else null end,'margin_percent',case when missing=0 and rev>0 then round(((rev-cost)*100/rev)::numeric,2) else null end,'missing_cost_lines',missing,'complete',missing=0);
end;$$;
revoke all on function get_profit_summary() from public,anon;grant execute on function get_profit_summary() to authenticated;
