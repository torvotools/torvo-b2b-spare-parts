...
declare u app_users%rowtype;l customer_demand_dealer_leads%rowtype;d customer_product_demands%rowtype;dealer_user uuid;cid uuid;begin
 ...
 -- Retry-safe: return the existing published direct campaign for this same lead/user instead of duplicating an alert.
 ...
 insert into app_push_outbox(inbox_id,push_device_id) select i.id,p.id from app_notification_inbox i join app_push_devices p on p.user_id=i.user_id and p.active=true where i.campaign_id=cid on conflict do nothing;
