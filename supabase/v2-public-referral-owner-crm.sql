...
language plpgsql security definer set search_path=public as $$declare s text:=upper(btrim(coalesce(p_status,'')));q text:='%'||upper(btrim(coalesce(p_search,'')))||'%';begin
 ...
 return query select r.id,r.referral_code,r.created_at,r.status,r.lead_source,c.full_name,coalesce(nullif(c.whatsapp,''),c.mobile),r.customer_pin_code,i.id,i.item_code,i.name,d.id,d.shop_name,r.contacted_at,r.converted_at,r.closed_at,r.owner_note
 from customer_dealer_referrals r join customer_contacts c on c.id=r.customer_id join catalog_items i on i.id=r.product_id join dealers d on d.id=r.dealer_id
 where (s='' or upper(coalesce(r.status,''))=s) and (p_search is null or btrim(p_search)='' or upper(coalesce(r.referral_code,'')) like q or upper(coalesce(c.full_name,'')) like q or coalesce(c.mobile,'') like replace(q,'%','%') or upper(coalesce(i.item_code,'')) like q or upper(coalesce(i.name,'')) like q or upper(coalesce(d.shop_name,'')) like q or coalesce(r.customer_pin_code,'') like replace(q,'%','%'))
...
language plpgsql security definer set search_path=public as $$declare a text:=upper(btrim(coalesce(p_action,'')));r customer_dealer_referrals%rowtype;n text:=nullif(btrim(coalesce(p_note,'')),'');begin
 perform torvo_assert_referral_crm_admin();
 ...
 if a='MARK_CONTACTED' then update customer_dealer_referrals set status='contacted',contacted_at=coalesce(contacted_at,now()),owner_note=coalesce(n,owner_note),updated_at=now() where id=p_referral_id;
 elsif a='MARK_CONVERTED' then update customer_dealer_referrals set status='converted',contacted_at=coalesce(contacted_at,now()),converted_at=coalesce(converted_at,now()),closed_at=coalesce(closed_at,now()),owner_note=coalesce(n,owner_note),updated_at=now() where id=p_referral_id;insert into dealer_referral_events(enquiry_id,dealer_id,event_type,product_id) select e.enquiry_id,r.dealer_id,'CONFIRMED_CONVERSION',r.product_id from dealer_referral_events e where e.dealer_id=r.dealer_id and e.product_id=r.product_id and e.event_type='REFERRAL_CREATED' order by e.created_at desc limit 1;
 elsif a='CLOSE' then update customer_dealer_referrals set status='closed',closed_at=coalesce(closed_at,now()),owner_note=coalesce(n,owner_note),updated_at=now() where id=p_referral_id;
 else update customer_dealer_referrals set status='dealer_selected',closed_at=null,converted_at=null,owner_note=coalesce(n,owner_note),updated_at=now() where id=p_referral_id;end if;
 return query select x.id,x.status,x.contacted_at,x.converted_at,x.closed_at from customer_dealer_referrals x where x.id=p_referral_id;
end$$;
