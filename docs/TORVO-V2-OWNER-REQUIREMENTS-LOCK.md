...
- Dealer login uses 10-digit registered mobile. First login/defined Dealer security verification uses WhatsApp OTP, then secure 4-digit PIN; forgot PIN and suspicious/new device require WhatsApp verification. Admin/Accountant secure desktop uses the separately approved Email ID + User ID + Email OTP flow.
